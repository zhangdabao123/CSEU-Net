...
def Unet(input_shape=(256,256,3), num_classes=21, backbone = "vgg", phi = 0):
    ...
    if backbone == "vgg":
        ...
        feat1, feat2, feat3, feat4, feat5 = Convnext(inputs, num_classes) 
    else:
    ...
    if phi >= 1 and phi <= 4:
        ...
        feat4 = attention[phi - 1](feat4, name='feat4')
        # feat5 = attention[phi - 1](feat5, name='feat5')
      
    channels = [64, 128, 256, 512]

    # 32, 32, 512 -> 64, 64, 512
    P5_up = attention[phi - 1](feat5, name='feat5')
    P5_up = UpSampling2D(size=(2, 2))(P5_up)
    ...
    P4_up = attention[phi - 1](P4, name='P4')
    P4_up = UpSampling2D(size=(2, 2))(P4_up)
    ...
    P3_up = attention[phi - 1](P3, name='P3')
    P3_up = UpSampling2D(size=(2, 2))(P3_up)
    ...
    P2_up = attention[phi - 1](P2, name='P2')
    P2_up = UpSampling2D(size=(2, 2))(P2_up)
    ...
    if backbone == "vgg":
        ...
        ConvNeXt_up = UpSampling2D(size=(2, 2))(P1)
        
        ConvNeXt_up = Conv2D(channels[0], 3, activation='relu', padding='same', kernel_initializer = RandomNormal(stddev=0.02))(ConvNeXt_up)
        ConvNeXt_up = Conv2D(channels[0], 3, activation='relu', padding='same', kernel_initializer = RandomNormal(stddev=0.02))(ConvNeXt_up)

        P1 = Conv2D(num_classes, 1, activation="softmax")(ConvNeXt_up)
    else:
        raise ValueError('Unsupported backbone - `{}`, Use vgg, resnet50.'.format(backbone))
